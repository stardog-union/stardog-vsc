...
module.exports = class Commander {
  ...
  insertSampleQuery() {
    ...
    const sampleQueryFiles = fs.readdirSync(sampleQueryDir);

    const sampleQueries = sampleQueryFiles.reduce((memo, file) => {
      const splitPaths = file.split('.');
      splitPaths.pop();
      const queryName = splitPaths.join('.');      
      return Object.assign(memo, {
        [queryName]: file,
      });
    }, {});

    this._window.showQuickPick(Object.keys(sampleQueries))
      .then(pickedQuery => {
        const pickedQueryPath = path.resolve(sampleQueryDir, sampleQueries[pickedQuery]);
        const queryText = fs.readFileSync(pickedQueryPath, 'utf-8');
        const { start, end } = this._window.activeTextEditor.selection;
        const insertionRange = new this._Range(start, end);
        const insertion = new this._TextEdit.replace(insertionRange, queryText);
